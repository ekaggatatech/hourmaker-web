...
def get_pricing_plans() -> str:
    ...
    Use this for pricing, cost, plans, seat limits, what's included, or plan feature questions.
    """
    try:
        content = _read_frontend("src", "data", "price.js")
        start_idx = content.find("plans: [")
        end_idx = content.find("comparisonFeatures: [")

        if start_idx == -1 or end_idx == -1:
            return "Sorry, I am unable to retrieve the pricing plans at the moment."

        plans = _parse_pricing_plans(content[start_idx:end_idx])
        if not plans:
            return "Sorry, I am unable to retrieve the pricing plans at the moment."

        formatted = _format_pricing_plans(plans)
        print(f"[TOOLS] Loaded {len(plans)} pricing plans")
        return formatted

    except Exception as e:
        print(f"[TOOLS] Failed to read pricing plans: {e}")
        return "Sorry, I am unable to retrieve the pricing plans at the moment."
