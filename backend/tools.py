...
def get_pricing_plans() -> str:
    """
    Returns Hourmaker plan prices and the features included in Free, Pro, and Enterprise.
    Always uses the bundled catalog so Render (backend-only deploys) cannot fall back
    to an old Basic / Pro Plus list.
    """
...
def _policy_knowledge():
    return (
        ...
        "that is the Docs portal on Pro and Enterprise. Free has company profile "
