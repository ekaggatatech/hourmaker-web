...
function compactBotMarkdown(text) {
  return String(text || "")
    .replace(/\r\n/g, "\n")
    .replace(/[ \t]+\n/g, "\n")
    .replace(/\n{3,}/g, "\n\n")
...
function hasMarkdownContent(node) {
  const items = Children.toArray(node).filter(Boolean);
  if (items.length === 0) return false;
  return items.some((child) => {
    if (typeof child === "string") {
      return child.replace(/\u00a0/g, " ").trim().length > 0;
    }
    if (child?.type === "br") return false;
    if (child?.props?.children != null) {
      return hasMarkdownContent(child.props.children);
    }
    return true;
  });
}
...
const markdownComponents = {
  ...
  p: ({ children }) =>
    hasMarkdownContent(children) ? <p>{children}</p> : null,
