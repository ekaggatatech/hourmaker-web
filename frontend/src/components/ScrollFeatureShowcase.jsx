...
function FeatureImageNav({
  ...
  const thumbHeightPercent = 100 / Math.max(1, features.length);
  const thumbTopPercent = activeIndex * thumbHeightPercent;

  return (
    <div className="flex flex-col items-center shrink-0 w-8 py-2" aria-label="Feature progress">
      ...
      <div 
        className="relative w-4 h-[min(250px,40vh)] bg-slate-200/50 rounded-full my-2 overflow-hidden cursor-pointer shadow-inner"
        onClick={(e) => {
          ...
          const targetIndex = Math.min(
            features.length - 1,
            Math.max(0, Math.floor(percentage * features.length))
          );
          onSelect(features[targetIndex].id);
        }}
      >
        <div
          className="absolute left-0 w-full bg-slate-400 rounded-full transition-all duration-300 ease-out hover:bg-slate-500"
          style={{
